...
func seedServices(s *Store) {
	...
	add(ServiceEntry{ID: "rp-registry", Name: "Rule-Pack Registry", Plane: "core", Kind: "service", TItems: []string{"rp-*"}, BaseURL: "http://localhost:8081", URLEnv: "RP_REGISTRY_URL", Description: "Registers, signs and serves rp-* rule-pack versions."})
	add(ServiceEntry{ID: "tin-graph", Name: "TIN Graph / Identity", Plane: "core", Kind: "service", TItems: []string{"T5", "T11"}, BaseURL: "http://localhost:8082", URLEnv: "TIN_GRAPH_URL", Description: "NIN=TIN / CAC-RC=TIN fusion, verification adapters, entity resolution."})
	add(ServiceEntry{ID: "rules-engine", Name: "Rules Engine", Plane: "core", Kind: "service", TItems: []string{"rp-*"}, BaseURL: "http://localhost:8083", URLEnv: "RULES_ENGINE_URL", Description: "Generic rp-* evaluation API (FastAPI)."})
	add(ServiceEntry{ID: "feature-store", Name: "Feature Store", Plane: "core", Kind: "service", TItems: []string{"T4"}, BaseURL: "http://localhost:8084", URLEnv: "FEATURE_STORE_URL", Description: "Offline/online feature materialisation."})
	add(ServiceEntry{ID: "ledger", Name: "Ledger (TigerBeetle)", Plane: "core", Kind: "service", TItems: []string{"T12"}, BaseURL: "http://localhost:8085", URLEnv: "LEDGER_URL", Description: "Double-entry ledger; dev in-memory TigerBeetle semantics."})
	add(ServiceEntry{ID: "notification", Name: "Notification", Plane: "core", Kind: "service", TItems: []string{"T5", "T12", "T14"}, BaseURL: "http://localhost:8086", URLEnv: "NOTIFICATION_URL", Description: "SMS/USSD/email/push via provider interfaces + simulators."})
	add(ServiceEntry{ID: "audit-evidence", Name: "Audit & Evidence (WORM)", Plane: "core", Kind: "service", TItems: []string{"all"}, BaseURL: "http://localhost:8087", URLEnv: "AUDIT_EVIDENCE_URL", Description: "Append-only audit log, WORM evidence store, TAT assembly."})
	add(ServiceEntry{ID: "geo", Name: "Geo Attribution", Plane: "core", Kind: "service", TItems: []string{"T6", "T11"}, BaseURL: "http://localhost:8088", URLEnv: "GEO_URL", Description: "Point→state/LGA/ward attribution (calls geo-rs)."})
	add(ServiceEntry{ID: "geo-rs", Name: "geo-rs (Rust)", Plane: "core", Kind: "service", TItems: []string{"T6", "T11"}, BaseURL: "http://localhost:8094", URLEnv: "GEO_RS_URL", Description: "Point-in-polygon + haversine prefilter engine."})
	add(ServiceEntry{ID: "consent", Name: "Consent (NDPA)", Plane: "core", Kind: "service", TItems: []string{"T5", "T14"}, BaseURL: "http://localhost:8089", URLEnv: "CONSENT_URL", Description: "NDPA consent capture, revocation and receipts."})
	add(ServiceEntry{ID: "reg-watch", Name: "Reg-Watch / Gates", Plane: "core", Kind: "service", TItems: []string{"G1", "G2", "G8"}, BaseURL: "http://localhost:8090", URLEnv: "REG_WATCH_URL", Description: "Gate & gazette monitor; board-authorised gate flips."})
	add(ServiceEntry{ID: "search-indexer", Name: "Search Indexer", Plane: "core", Kind: "service", TItems: []string{"all"}, BaseURL: "http://localhost:8091", URLEnv: "SEARCH_INDEXER_URL", Description: "Outbox→OpenSearch indexer (dev: local JSON index)."})
	add(ServiceEntry{ID: "settlement", Name: "Settlement / Recon", Plane: "core", Kind: "service", TItems: []string{"T12"}, BaseURL: "http://localhost:8092", URLEnv: "SETTLEMENT_URL", Description: "PSSP 3-way reconciliation (platform vs PSSP vs treasury)."})
	add(ServiceEntry{ID: "edge-policy", Name: "Edge Policy (APISIX)", Plane: "core", Kind: "service", TItems: []string{"all"}, BaseURL: "http://localhost:8093", URLEnv: "EDGE_POLICY_URL", Description: "Route policy distribution + WAF mode control."})
	...
	add(ServiceEntry{ID: "einvoicing", Name: "E-Invoicing (MBS)", Plane: "compliance", Kind: "service", TItems: []string{"T1", "T2"}, BaseURL: "http://localhost:8101", URLEnv: "EINVOICING_URL", Description: "UBL 2.1/Peppol BIS mapping, CSID signing, pre-clearance."})
	add(ServiceEntry{ID: "rev360", Name: "Rev360 Reconciliation", Plane: "compliance", Kind: "service", TItems: []string{"T3"}, BaseURL: "http://localhost:8102", URLEnv: "REV360_URL", Description: "Reconciliation workbench + defect-class rules engine."})
	add(ServiceEntry{ID: "wht", Name: "WHT 2024", Plane: "compliance", Kind: "service", TItems: []string{"T7"}, BaseURL: "http://localhost:8103", URLEnv: "WHT_URL", Description: "Withholding tax evaluation via rp-wht-2024, credit ledger."})
	add(ServiceEntry{ID: "tp-cbcr", Name: "TP / CbCR", Plane: "compliance", Kind: "service", TItems: []string{"T8"}, BaseURL: "http://localhost:8104", URLEnv: "TP_CBCR_URL", Description: "OECD CbCR XML, master/local file assembly."})
	add(ServiceEntry{ID: "pos-vat", Name: "POS VAT", Plane: "compliance", Kind: "service", TItems: []string{"T6"}, BaseURL: "http://localhost:8105", URLEnv: "POS_VAT_URL", Description: "POS receipt ingest, attribution, offline spool."})
	add(ServiceEntry{ID: "etr", Name: "ETR / GloBE", Plane: "compliance", Kind: "service", TItems: []string{"T9"}, BaseURL: "http://localhost:8106", URLEnv: "ETR_URL", Description: "ETR compute engine, GIR/filing-pack builder."})
	add(ServiceEntry{ID: "vasp-carf", Name: "VASP / CARF", Plane: "compliance", Kind: "service", TItems: []string{"T10"}, BaseURL: "http://localhost:8107", URLEnv: "VASP_CARF_URL", Description: "VASP trade ingest, cost-basis, CARF XML builder."})
	add(ServiceEntry{ID: "case-mgmt", Name: "Practitioner Case Mgmt", Plane: "compliance", Kind: "service", TItems: []string{"T13p"}, BaseURL: "http://localhost:8108", URLEnv: "CASE_MGMT_URL", Description: "Matters, documents, deadlines, evidence packs."})
	...
	add(ServiceEntry{ID: "onboarding", Name: "Onboarding", Plane: "inclusion", Kind: "service", TItems: []string{"T5"}, BaseURL: "http://localhost:8201", URLEnv: "ONBOARDING_URL", Description: "wf-onb-*, NIMC adapter, offline capture sync."})
	add(ServiceEntry{ID: "presumptive", Name: "Presumptive Tax (PSM)", Plane: "inclusion", Kind: "service", TItems: []string{"T12"}, BaseURL: "http://localhost:8202", URLEnv: "PRESUMPTIVE_URL", Description: "Payment intents, PSSP adapter, certificates, agent float."})
	add(ServiceEntry{ID: "education", Name: "Tax Education", Plane: "inclusion", Kind: "service", TItems: []string{"T14"}, BaseURL: "http://localhost:8203", URLEnv: "EDUCATION_URL", Description: "Calculators with citations, FAQ corpus, embed SDK."})
	add(ServiceEntry{ID: "ussd-gateway", Name: "USSD Gateway", Plane: "inclusion", Kind: "service", TItems: []string{"T5", "T12"}, BaseURL: "http://localhost:8204", URLEnv: "USSD_GATEWAY_URL", Description: "Menu-graph session engine, aggregator webhook simulator."})
	add(ServiceEntry{ID: "agent-pwa", Name: "Agent PWA", Plane: "inclusion", Kind: "app", TItems: []string{"T5"}, BaseURL: "http://localhost:5175", URLEnv: "AGENT_PWA_URL", HealthPath: "/", Description: "Offline-first agent capture app."})
	add(ServiceEntry{ID: "operator-pwa", Name: "Operator PWA", Plane: "inclusion", Kind: "app", TItems: []string{"T12", "T14"}, BaseURL: "http://localhost:5176", URLEnv: "OPERATOR_PWA_URL", HealthPath: "/", Description: "Operator self-service app."})

	// ---- gov plane (Sovereign Zone) ----
	add(ServiceEntry{ID: "analytics", Name: "Analytics / Scoring", Plane: "gov", Kind: "service", TItems: []string{"T4", "T15"}, BaseURL: "http://localhost:8301", URLEnv: "ANALYTICS_URL", Description: "Lakehouse-lite, daily scoring, NSW ingest."})
	add(ServiceEntry{ID: "jrb", Name: "JRB Service", Plane: "gov", Kind: "service", TItems: []string{"T11"}, BaseURL: "http://localhost:8302", URLEnv: "JRB_URL", Description: "Authority registry, EOI, attribution feeds."})
	add(ServiceEntry{ID: "ombud", Name: "Tax Ombud", Plane: "gov", Kind: "service", TItems: []string{"T13i"}, BaseURL: "http://localhost:8303", URLEnv: "OMBUD_URL", Description: "Case registry, deposit tracker, evidence packs."})
	add(ServiceEntry{ID: "enclave-gateway", Name: "Enclave Gateway", Plane: "gov", Kind: "service", TItems: []string{"F1-F8"}, BaseURL: "http://localhost:8304", URLEnv: "ENCLAVE_GATEWAY_URL", Description: "Audited API gateway — sole north-south path; WORM receipts."})
	add(ServiceEntry{ID: "gov-console", Name: "Gov Console", Plane: "gov", Kind: "console", TItems: []string{"T4", "T11", "T13i"}, BaseURL: "http://localhost:5177", URLEnv: "GOV_CONSOLE_URL", HealthPath: "/", Description: "NRS/JRB/state-IRS/Ombud console."})
}
