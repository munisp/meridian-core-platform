                ...
                <div key={s.id} className="card p-5 flex flex-col">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="text-xs text-stone-600 font-mono mt-0.5">{s.id} · v{s.version}</div>
                    </div>
                    <HealthBadge status={s.health_status} />
                  </div>
                  {s.description && <p className="mt-2 text-sm text-stone-600 flex-1">{s.description}</p>}
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {s.t_items.map((t) => <Badge key={t} tone="sand">{t}</Badge>)}
                    <Badge tone="clay">{s.kind}</Badge>
                  </div>
                  <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-3">
                    <a href={s.base_url} target="_blank" rel="noreferrer" className="text-xs text-brand-700 hover:text-brand-800 font-mono truncate max-w-[60%]">
                      {s.base_url}
                    </a>
                    <button
